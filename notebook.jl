...
# ╔═╡ f2385d10-af74-11ec-2333-072da28c3ab7
using CSV, DataFrames, RDatasets, StatsBase, Statistics, PlutoUI, VegaLite, Plots, Flux

# ╔═╡ 4bd8aa5b-e825-4461-b1f5-b5cf2001bce1
df = CSV.read("C:/Users/Prekansius James/Desktop/Real estate.csv", DataFrame)

# ╔═╡ e633f366-cc84-43d4-97d6-7f2915416d2d
size(df)

# ╔═╡ 2a90b049-15d0-48ca-84c5-916beff88bb4
describe(df)

# ╔═╡ acf2ac02-3262-4a75-88b3-a229e80ea08e
cor(Matrix(df))

# ╔═╡ 70b14bd7-ada9-41b5-8300-303ffe4fa9f9
X = df[:, 1:8]

# ╔═╡ 8520bc76-77ba-4c2a-b03a-64f0d3d9ca04
X_mat = Matrix(X)

# ╔═╡ e113607b-36f6-4d0a-9030-8cdcaaaebf72
Y = df[:, 2]

# ╔═╡ 03e821f5-206a-405b-814a-68653b9d009d
Y_mat = Vector{Float64}(Y)

# ╔═╡ b482623b-732a-42b8-b054-39121e27180e
training_size = 0.7

# ╔═╡ 2874e99c-e67a-4e83-ae8f-6998b7ea72b7
all_data_size = size(X_mat)[1]

# ╔═╡ 37b0c125-47ea-493a-8ac7-05ff6767c1b5
training_index = trunc(Int, training_size * all_data_size)


# ╔═╡ 0025cb62-5c56-4d27-9843-82129864cc4f
X_mat_train = X_mat[1:training_index, :]


# ╔═╡ 07fe178c-5f9b-4ecf-89e5-68d1e89262ef
X_mat_test = X_mat[training_index+1:end, :]

# ╔═╡ 4cde412d-5fe6-4c41-bf1c-e7fc9fa2781b

Y_mat_train = Y_mat[1:training_index]

# ╔═╡ 6fa65e28-8c5d-4723-af97-033b013c7c4a

Y_mat_test = Y_mat[training_index+1:end]


# ╔═╡ ccf9a263-9dbd-431a-be28-a131eada6986
function get_scaling_params(init_features)::Tuple{Matrix, Matrix}
...
# ╔═╡ 4a58e2c1-1f44-48e8-9653-b0e93aa53ebe
...
# ╔═╡ 2fd3aefc-dcdf-4ea8-918a-a9acab4ab080
scaling_params = get_scaling_params(X_mat_train)

# ╔═╡ 24f7698e-6c20-4acd-9176-7ecc5da03a3f
scaled_training_features = scale_features(X_mat_train, scaling_params)


# ╔═╡ 821fc75b-61f6-44d4-a905-2b57ffa19136
scaled_testing_features = scale_features(X_mat_test, scaling_params)

# ╔═╡ 1c229f72-1242-4aa6-85ac-75bd1e9a7cc6
function unpack_matrix(A::Matrix)
	return[A[:,c] for c in 1:size(A,2)]
end

# ╔═╡ 8ddc32b3-ad0a-4b2d-b0c1-ea933e1c4dcc
...
# ╔═╡ 5a146444-c94a-497a-9d1e-0261ac2eb63e
unpacked_scaled_training_features = slicematrix(scaled_training_features)

# ╔═╡ 8b6200e9-2f5f-4897-8f71-fbffd924275c
unpacked_scaled_testing_features = slicematrix(scaled_testing_features)

# ╔═╡ 1c687e99-4358-4628-8a9c-dced71a464ec
function train_model(x_train, y_train, n_epochs, optimiser)
feature_count = size(x_train[1])[1]
	predict = Chain(
		Dense(feature_count, 77, σ),
		Dense(77,1),
		softmax
	)

	loss_fn(x,y) = Flux.Losses.mse(predict(x), y)

	data = zip(x_train, y_train)
	parameters = Flux.params(predict)
	loss_over_time = []
	for epoch in 1:n_epochs
		Flux.train!(loss_fn, parameters, data, optimiser)

		println("Epoch: $epoch")
	end

	return predict, loss_fn, loss_over_time

end

# ╔═╡ 1915954c-ad54-4b67-aebf-5011f389187e
predict, loss_fn, loss_over_time = train_model(unpacked_scaled_testing_features,Y_mat_train, 100, ADAM())

# ╔═╡ b7d11a26-037c-4771-870b-23abbef5279e
Matrix(scaled_training_features)

# ╔═╡ 19609105-d020-47fe-bdd1-667f39e01171
function get_predictions(x)::Vector
	preds = predict.(x)
	return trunc.(Int64, collect(Iterators.flatten(preds)))
end

# ╔═╡ 326ca0cd-99c2-4fd9-8f3f-bc081500ff02
training_predictions = get_predictions(unpacked_scaled_training_features)

# ╔═╡ 46921967-8e95-4ceb-836b-11b0b75b0fcf
testing_predictions = get_predictions(unpacked_scaled_testing_features)

# ╔═╡ 90d74067-4498-48e1-b51a-78cc2193f7cf

function measure(actual::Vector, predicted::Vector)
    confusion_matrix = MLBase.roc(actual, predicted)

	correctly_classified = actual .== predicted
	accuracy::Int64 = round(mean(correctly_classified) * 100)
	with_terminal() do
...
# ╔═╡ c45c2a40-59b6-4ed6-88c2-7968b9152403
size(Y_mat_train)

# ╔═╡ 5084c2dd-7c30-49dd-9d72-003ae82513cf
size(training_predictions)

# ╔═╡ f1b45be3-6f18-4084-9e8c-9d3f9b094e13
measure(Y_mat_train, training_predictions)

# ╔═╡ feb7f263-7cba-45fc-84fa-f4dd7f57d53d
measure(Y_mat_test, testing_predictions)
